...
const roles = [
    ...
    { id: "ProjectImplManager", label: "Project Impl. Manager", icon: "fa-diagram-project" },
...
function renderRoles() {
    ...
    document.querySelectorAll('.role-card').forEach(card => {
        card.addEventListener('click', () => {
            ...
            taskInput.value = "";
            taskInput.placeholder = `e.g., I need a prompt to detect subtle Hate Speech in gaming chat.\n\ne.g., Create a policy guideline for AI-generated misinformation.`;
...
/* -------------------------------------------------------------------------- */
/* [4] 페르소나 생성 (T.C.R.E.I & Short Questions)                            */
/* -------------------------------------------------------------------------- */
...
async function generatePersonas() {
    ...
    // 🔥 핵심 변경사항: 질문 길이 제한(300자) 및 T.C.R.E.I 프레임워크 적용
    const prompt = `
    ...
    1. **Short Questions:** Every question must be **under 300 characters**. Be direct and concise. No fluff.
    2. **Smart Suggestions:** Provide options in this format: ||Option 1||Option 2||Option 3||
    
    [FINAL PROMPT STANDARD (T.C.R.E.I)]
    The final output generated by these personas MUST follow this structure:
    - **T**houghtfully: Define Persona & Format clearly.
    - **C**reate: Include full Context.
    - **R**eally: Add Reference materials/Input placeholders.
    - **E**xcellent: Add Evaluation Criteria (how to judge success).
    - **I**nputs: Show it's a result of iterative refinement.
    ...
    [
        {
            ...
            "system_instruction": "You are [Persona]. Build a T&S prompt for '${state.task}'. \n\nRULES:\n1. Ask ONE question at a time.\n2. Keep questions **UNDER 300 CHARACTERS**.\n3. Provide 3-4 suggestions using ||Option|| format.\n4. When outputting the final prompt, adhere strictly to the T.C.R.E.I framework structure.",
            "first_message": "Let's define the **Scope**. What specific harm area are we targeting?\n\n||Harassment||Fraud||Child Safety||Misinformation||"
        }
    ]
    `;
...
/* -------------------------------------------------------------------------- */
/* [5] 채팅 엔진 (Strict Format & Length Control)                             */
/* -------------------------------------------------------------------------- */
...
function startChat(idx) {
    ...
    // 🔥 시스템 프롬프트: T.C.R.E.I 구조 강제 및 질문 길이 재강조
    state.chatMessages = [
        { 
            role: "system", 
            content: state.selectedPersona.system_instruction + 
            "\n\n[GLOBAL RULES]\n1. Question Length: MAX 300 characters. Be extremely concise.\n2. Always provide ||Option|| suggestions.\n\n[FINAL OUTPUT FORMAT (Markdown)]\nWhen you have enough info, generate the prompt in ```markdown``` block following this T.C.R.E.I structure:\n\n# Role (Thoughtfully)\n...\n# Context (Create)\n...\n# Input Data & Reference (Really)\n...\n# Evaluation Criteria (Excellent)\n...\n# Task (Inputs)\n..." 
        }
    ];
...
async function sendMessage(manualText = null) {
    ...
    input.value = '';
    input.placeholder = "Type specific detail..."; 
    ...
    const loadingId = addMessageToUI("assistant", "Refining...", true);
...
function addMessageToUI(role, text, isTemp = false, options = []) {
    ...
    if (role === 'assistant' && !isTemp) {
        const display = text.replace(/```(?:markdown|prompt)?\n([\s\S]*?)```/g, 
            '<div class="bg-indigo-50 border border-indigo-200 p-3 rounded-lg text-xs text-indigo-700 cursor-help"><i class="fa-solid fa-code"></i> Prompt Optimized (Check Preview)</div>'
        );
    ...
    if (!barContainer) {
        ...
        barContainer.innerHTML = `
            <div class="flex justify-between text-[10px] text-slate-400 mb-1 font-bold uppercase">
                <span>Building Prompt (T.C.R.E.I)...</span>
        ...
        <div class="fade-in space-y-6">
            <div>
                <div class="flex items-center gap-2 mb-2">
                    <span class="text-xs font-bold text-slate-500 uppercase">T&S Prompt Code (T.C.R.E.I)</span>
                </div>
...
${promptCode.replace(/</g, '&lt;').replace(/>/g, '&gt;')}
    ...
    try {
        const simulationResult = await callChat([
            { role: "system", content: "You are a Content Moderation AI. Execute the safety policy prompt exactly." },
        ...
        document.getElementById('simulationResultArea').innerHTML = `
            <div class="border-t pt-4 fade-in">
                <div class="mb-3 flex items-center gap-2">
                    <span class="text-xs font-bold text-indigo-600 uppercase">Simulated AI Response</span>
